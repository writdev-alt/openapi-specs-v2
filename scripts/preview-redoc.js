...
function rebuildBundle() {
  ...
  isRebuilding = true;
  console.log('\n🔄 Rebuilding OpenAPI specification...');
  try {
    ...
    if (stats.isDirectory()) {
      ...
      if (!watchPath.endsWith('openapi.yaml')) {
        fs.watch(watchPath, (eventType, filename) => {
          if (isRebuilding) return;
          console.log(`\n📝 File changed: ${path.basename(watchPath)}`);
          scheduleRebuild();
        });
      }
    }
...
server.listen(PORT, () => {
  console.log(`\n🚀 Redoc preview server running on http://localhost:${PORT}`);
  console.log('📚 Using open-source Redoc library');
  console.log('🔄 Hot reload enabled - watching for file changes...');
  console.log('👁️  Open your browser to view the API documentation\n');
});
